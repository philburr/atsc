...
TEST_CASE("ATSC Field Sync", "[opencl,field_sync]") {
    ...
    auto field = std::make_unique<atsc_field_symbol>();
    ...
    auto saved_symbols = atsc_reserved_symbols();
    ...
    auto test_data = std::make_unique<atsc_field_symbol>();
    atsc.to_host(test_data->data(), input, test_data->size() * sizeof((*test_data)[0]), event);
    atsc.cl_free(input);

    // Host
    auto valid_data = std::make_unique<atsc_field_symbol_padded>();
    for (size_t i = 0; i < field->size(); i++) {
        (*valid_data)[i] = (*field)[i];
    }

    auto host_fs = atsc_field_sync<void>();
    host_fs.process_field(*valid_data, saved_symbols);
